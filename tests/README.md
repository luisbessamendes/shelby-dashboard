...
- Confirmed renovations and closures are distinct cohorts. Event months gate the selected LTM endpoint; all actual financial records remain included.
...
- The second register column is reviewed for 2026, not future years. Update the register when statuses or periods change, preserving financial records and retesting each bridge independently.
...
- `node --env-file=.env.local --import ./tests/register-typescript.mjs tests/perimeter-live.mjs` performs a read-only audit of the uploaded data and five LTM endpoints. Set `PERIMETER_WORKBOOK` to the source workbook path to also verify its hash and every classification. No workbook or database writes.
