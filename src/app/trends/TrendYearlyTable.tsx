...
export default function TrendYearlyTable({ data, basis, month }: TrendYearlyTableProps) {
  ...
  const getDelta = (curr: number | null | undefined, prev: number | null | undefined, isRatio = false) => {
          ...
          <tbody>
            {data.map((row) => (
              <tr key={row.year}>
                ...
                <td className="numeric text-center">{formatCurrency(row.metrics.totalTurnover)}</td>
            ...
            {latest && previous && (
              <tr style={{ background: 'var(--accent-primary-dim)', borderTop: '2px solid var(--border-glass)' }}>
                ...
                <td className={`numeric text-center ${getDelta(latest.metrics.totalTurnover, previous.metrics.totalTurnover)! >= 0 ? 'cell-positive' : 'cell-negative'}`}>
                  {formatTrend(getDelta(latest.metrics.totalTurnover, previous.metrics.totalTurnover))}
                </td>
