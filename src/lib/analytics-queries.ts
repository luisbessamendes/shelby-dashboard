...
async function getFilteredData(filters: FilterState): Promise<{
...
export async function buildAnalyticsContext(filters: FilterState): Promise<AnalyticsContext> {
  ...
  // Generate a highly compact CSV of all stores to give the AI full resolution into all operational costs
  const csvHeader = "Store,Concept,Region,Sales(€),EBITDA(€),EBITDA%,CAPEX(€),CIT(€),FCFF(€),Rents%,Staff%,Utils%,Maint%,Bank%,Vat%,Admin%\n";
  const csvRows = storeList.map(s => {
    const rNum = (val: number | null | undefined) => (val != null ? Number(val.toFixed(2)) : 0);
    const rPct = (val: number | null | undefined) => (val != null ? Number((val * 100).toFixed(1)) + '%' : '0%');
    
    return `"${s.store}","${s.concept}","${s.region}",${s.totalSales.toFixed(0)},${s.totalEbitda.toFixed(0)},${rPct(s.ebitdaPct)},${s.totalCapex.toFixed(0)},${s.totalCit.toFixed(0)},${s.totalFcff.toFixed(0)},${rPct(s.rentsPct)},${rPct(s.staffPct)},${rPct(s.utilitiesPct)},${rPct(s.maintenancePct)},${rPct(s.bankingCostsPct)},${rPct(s.vatPct)},${rPct(s.adminCostsPct)}`;
  }).join("\n");
