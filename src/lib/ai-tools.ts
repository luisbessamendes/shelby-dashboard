...
type AiToolArgs = {
  ...
  concept?: string;
  region?: string;
  metric?: 'turnover' | 'sales' | 'ebitda' | 'fcff' | 'capex';
  limit?: number;
  order?: 'asc' | 'desc';
};
...
export const AI_TOOLS = [
  {
    ...
    function: {
      ...
      description: 'Get turnover, gross sales, EBITDA, ratios, and other metrics for a specific time period and filters (Concept, Region, etc.). Use this for Year-over-Year (YoY) comparisons or general portfolio performance queries.',
      parameters: {
        type: 'object',
        properties: {
  ...
  },
  {
    ...
    function: {
      ...
      description: 'Get the top or bottom performing stores based on a specific metric (turnover, sales, ebitda, fcff, capex) for a given period.',
      parameters: {
        type: 'object',
        properties: {
          year: { type: 'number' },
          month: { type: 'number' },
          metric: { type: 'string', enum: ['turnover', 'sales', 'ebitda', 'fcff', 'capex'] },
          limit: { type: 'number' },
 ...
 */
export async function executeAiTool(name: string, args: AiToolArgs) {
  ...
  switch (name) {
    ...
    case 'get_store_rankings': {
      ...
      const key = metricMap[args.metric ?? 'turnover'];
